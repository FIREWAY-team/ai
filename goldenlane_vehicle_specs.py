...
#
# 2026-09-12 AI Hub 파인튜닝 클래스 체계로 갱신 — 버스/트럭 계열을 세분화 유지한다
# (소형버스·대형버스, 트럭·대형 트레일러를 합치면 실제 폭 차이가 캘리브레이션
# 오차로 들어가서 안 합침). 폭 값은 일반적인 차종별 평균치이며 실측값이 아니다 —
# 실측 데이터가 쌓이면 조정 대상(정확도개선방안 A-1/A-3와 같은 성격의 초기값).
